...
namespace AZMLib
{
    ...
    public class AZMCombiner : IDisposable
    {
        ...
        private bool _detectingChainCompleted = false;

        public bool IsPSIMSSBOutputEnabled { get; set; } = false;

        public bool USBLRawDataEventEnabled { get; set; } = false;
        ...
        readonly WPManager wpManager = new WPManager();
        ...
        AZMAntennaCorrector antCorrector;
        AngularCalibrationManager acManager = new AngularCalibrationManager();
        public bool AngularCalibration { get; private set; } = false;
        double _acStAngle_deg = 340;
        double _acNdAngle_deg = 20;
        double _acStep_deg = 0.5;
        int _acNPoints = 512;
        REMOTE_ADDR_Enum _acResponderAddr = REMOTE_ADDR_Enum.REM_ADDR_1;
        ...
        public double DataLatency_ms { get; set; } = 400.0; // 440-460 .. 480-500
        private readonly CompassHistory _compassHistory = new CompassHistory();
        ...
        public AZMCombiner(AZMSettingsContainer settings)
        {
            ...
            wpManager.IsAutoSoundSpeed = double.IsNaN(settings.speedOfSound);
            if (!wpManager.IsAutoSoundSpeed)
                wpManager.SoundSpeed = settings.speedOfSound;

            wpManager.IsAutoSalinity = false;
            wpManager.Salinity = Salinity_PSU;

            IsPSIMSSBOutputEnabled = settings.IsPSIMSSBOutputEnabled;
            ...
            antCorrector = new AZMAntennaCorrector();
        ...
        private void ValidateMainParameters(AZMSettingsContainer settings)
        {
            ...
            if (AZM.IsSosMps(settings.speedOfSound))
                SpeedOfSound_mps = settings.speedOfSound;
            else
                SpeedOfSound_mps = double.NaN;
        }

        private void InitializeFilters(AZMSettingsContainer settings)
        {
        ...
        private void InitializeAuxiliaryConnections(AZMSettingsContainer settings)
        {
        ...
        private void CheckChainCompleted()
        {
            if (_detectingChainCompleted) return;

            if (!AZMDetected) return;

            if (IsUseAUX1 && !AUX1Detected) return;

            if (IsUseAUX2 && !AUX2Detected) return;

            _detectingChainCompleted = true;
            LogInfo("Device chain completed (AZM → AUX1 → AUX2)");
            DetectingChainCompletedHandler?.Invoke(this, EventArgs.Empty);
        }
        ...
        public bool Disconnect()
        {
            ...
            _detectingChainCompleted = false;
        ...
        public void AUX1Init(BaudRate baudrate, bool alternative = false)
        {
            if (!IsUseAUX1)
            {
                ...
                aux1Port.HeadingUpdated += (o, e) =>
                {
                    state.Heading_deg.Value = aux1Port.Heading;
                    _compassHistory.Add(aux1Port.Heading);
                };
        ...
        public void AUX2Init(BaudRate baudrate)
        {
            if (!IsUseAUX2)
            {
                ...
                aux2Port.DetectedChanged += (o, e) => CheckChainCompleted();
                aux2Port.HeadingUpdated += (o, e) =>
                {
                    state.Heading_deg.Value = aux2Port.Heading;
                    _compassHistory.Add(aux2Port.Heading);
                };
                aux2Port.LogEventHandler += (o, e) => LogEventHandler?.Invoke(o, e);
            }
        }
        ...
        public void AngularCalibrationStart(double fromAngle_deg, double toAngle_deg, double step_deg, int npoints, REMOTE_ADDR_Enum raddr)
        {
            ...
            _acStAngle_deg = fromAngle_deg;
            _acNdAngle_deg = toAngle_deg;
            _acStep_deg = step_deg;
            _acNPoints = npoints;
            _acResponderAddr = raddr;

            acManager.X_Offset_m = x_offset_m;
            acManager.Y_Offset_m = y_offset_m;
        ...
        public void ApplyCalibrationTable(double[] encoderAngles, double[] errors)
        {
            try
            {
                antCorrector.LoadCalibration(encoderAngles, errors);
            }
            catch (Exception ex)
            {
                LogError($"Failed to loat calibration table ({serialOutput?.PortName}): {ex.Message}");
            }
        }
        ...
        public bool LocationOverrideUpdateHeading(double hdn_deg)
        {
            if (_disposed || !pTimer.Enabled) return false;

            if (AZM.IsHdnDeg(hdn_deg))
            {
                HeadingOverride = hdn_deg;
                return true;
            }
        ...
        private string BuildPSIMSSB(ResponderBeacon remote, DateTime timestamp)
        {
            // Проверяем, что всё инициализировано
            if (!state.Lat_deg.IsInitializedAndNotObsolete ||
                !state.Lon_deg.IsInitializedAndNotObsolete ||
                !state.Heading_deg.IsInitializedAndNotObsolete ||
                !remote.Lat_deg.IsInitialized ||
                !remote.Lon_deg.IsInitialized)
                return string.Empty;

            // Время в формате hhmmss.ss
            string timeStr = timestamp.ToString("HHmmss.ff", CultureInfo.InvariantCulture);

            // ID транспондера: B + номер (B01, B02...)
            string btpId = "B" + AZM.Addr2Str(remote.Address).PadLeft(2, '0');

            // Вычисляем относительные координаты в системе North-up
            double spLatRad = Algorithms.Deg2Rad(state.Lat_deg.Value);
            double spLonRad = Algorithms.Deg2Rad(state.Lon_deg.Value);
            double epLatRad = Algorithms.Deg2Rad(remote.Lat_deg.Value);
            double epLonRad = Algorithms.Deg2Rad(remote.Lon_deg.Value);

            Algorithms.GetDeltasByGeopoints_WGS84(spLatRad, spLonRad, epLatRad, epLonRad,
                out double deltaLat_m, out double deltaLon_m);

            // X_north = delta_lat (север), Y_east = delta_lon (восток)
            // Поворачиваем в систему Heading-up
            double headingRad = Algorithms.Deg2Rad(state.Heading_deg.Value);
            double cosH = Math.Cos(headingRad);
            double sinH = Math.Sin(headingRad);

            // X (вперёд) = north * cos(H) + east * sin(H)
            // Y (вправо) = -north * sin(H) + east * cos(H)
            double x_m = deltaLat_m * cosH + deltaLon_m * sinH;
            double y_m = -deltaLat_m * sinH + deltaLon_m * cosH;

            // Z = глубина ответчика (положительная вниз)
            double z_m = remote.Depth_m.IsInitialized ? remote.Depth_m.Value : 0.0;

            /*
             * Вариант для North-up
             
            double x_m = deltaLat_m;  // север
            double y_m = deltaLon_m;  // восток

            string sentence = string.Format(CultureInfo.InvariantCulture,
                "PSIMSSB,{0},{1},A,,C,N,M,{2:F02},{3:F02},{4:F02},0.0,N,,",
                timeStr, btpId, x_m, y_m, z_m);
            *
            */

            // Формируем строку
            // $PSIMSSB,hhmmss.ss,Btp,A,,C,H,M,x.xx,y.yy,z.zz,0.0,N,,*cc
            string sentence = string.Format(CultureInfo.InvariantCulture,
            ...
            return nmeaLine + "*" + checksum.ToString("X2");
        }
        ...
        private void ProcessRemoteUSBL(NDTAReceivedEventArgs e)
        {
            ...
            {
                remote.Azimuth_deg.Value = antCorrector.CorrectAngle(e.HAngle_deg);
                is_a = true;
            }
            ...
            if (is_a && is_srp)
            {
                ...
                {
                    double cHeading = _compassHistory.GetHeadingAtOffset(DataLatency_ms);

                    if (AngularCalibration && (e.Address == _acResponderAddr))
                    {
                        acManager.AddPoint(
                            DateTime.Now,
                            cHeading,
                            ...
                            remote.SRangeProjection_m.Value,
                            remote.PTime_s.Value,
                            state.StDepth_m.Value,
                            remote.Depth_m.Value);

                        if (acManager.Count >= _acNPoints)
                        {
                            phi_deg = acManager.Calibrate_Phi(_acStAngle_deg, _acNdAngle_deg, _acStep_deg);
                            AngularCalibration = false;

                            LogInfo($"Angular calibration finished, estimated value is {phi_deg:F01}°");
                        }
                    }

                    AZMMath.PolarCS_ShiftRotate(cHeading, phi_deg,
                        remote.Azimuth_deg.Value,
                        remote.SRangeProjection_m.Value,
            ...
            if (IsPSIMSSBOutputEnabled &&
                IsStationLocationAndHeadingValid() &&
                remote.AAzimuth_deg.IsInitialized &&
                remote.ADistance_m.IsInitialized)
            {
                string psimssb = BuildPSIMSSB(remote, DateTime.UtcNow);
                if (!string.IsNullOrEmpty(psimssb))
                    OutputHandler?.Rise(this, new StringEventArgs(psimssb));
            }
        ...
        public EventHandler? DetectingChainCompletedHandler;
